...
const AdminProducts = () => {
  ...
  const loadProducts = useCallback(async () => {
    const data = await mockApi.getAdminProducts({ keyword, categoryId });
  ...
  const handleSave = async (product: Product) => {
    ...
    try {
      await mockApi.updateProduct(product.id, { price, stock });
      setDrafts((prev) => {
        const next = { ...prev };
  ...
  const handleDelete = async (product: Product) => {
    if (!confirm(`确认删除商品「${product.name}」？该操作不可恢复。`)) return;
    setPendingId(product.id);
    try {
  ...
  return (
    <div className="admin-products" id="admin-products">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-desc">价格与库存可直接在表格内修改后保存</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowForm((open) => !open)}>
          ...
          <div className="admin-form-grid">
            <div className="admin-form-field">
              ...
              <input
                ...
                placeholder="如：高山红心猕猴桃 12 枚"
                required
              />
            </div>
            <div className="admin-form-field">
              <label htmlFor="new-price">售价（元）*</label>
              <input
                id="new-price"
                type="number"
                min="0.1"
            ...
            <div className="admin-form-field">
              <label htmlFor="new-stock">库存</label>
              <input
                id="new-stock"
                type="number"
                min="0"
            ...
            <div className="admin-form-field">
              <label htmlFor="new-unit">计量单位</label>
              <input
                id="new-unit"
                value={form.unit}
            ...
            <div className="admin-form-field">
              <label htmlFor="new-origin">产地</label>
              <input
                id="new-origin"
                value={form.origin}
            ...
            <div className="admin-form-field">
              <label htmlFor="new-category">商品分类 *</label>
              <select
                id="new-category"
            ...
            <div className="admin-form-field span-3">
              <label htmlFor="new-desc">商品描述</label>
              <textarea
                id="new-desc"
                rows={2}
                      ...
                      <td>
                        <div className="admin-table-product">
                          ...
                          <div>
                            ...
                            <small>{product.origin} · {product.sellerName}</small>
                          </div>
                        </div>
                      </td>
